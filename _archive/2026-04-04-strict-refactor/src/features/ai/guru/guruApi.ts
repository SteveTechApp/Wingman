...
function buildContextualQuestion(question: string, ctx: GuruContext): string {
  const note = tidy(ctx.notes);
...
export async function askGuru(question: string, ctx: GuruContext): Promise<GuruAnswer> {
  ...
  const contextualQuestion = buildContextualQuestion(q, ctx);
  const knowledge = await assessGuruKnowledge(contextualQuestion, ctx.mode);
  ...
  const baseSources = coreSourcesForQuestion(contextualQuestion);
  ...
  const intelligence = await assessQuestionIntelligence(contextualQuestion);
  const topRecords = intelligence.records.slice(0, 3);
  const directCapabilityAnswer = buildCapabilityLookupAnswer(contextualQuestion, intelligence.records);
