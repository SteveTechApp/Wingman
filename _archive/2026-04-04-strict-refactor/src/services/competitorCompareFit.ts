...
function _asPortMap(ports?: CatalogPortCount[]): Map<string, number> {
...
function _overlapRatio(left: string[], right: string[]): number {
...
function _scoreCapabilityFlags<T extends object>(
  label: string,
  competitorProfile: T | undefined,
  wyrestormProfile: T | undefined,
  definitions: ReadonlyArray<{ key: keyof T; label: string }>,
...
function _scoreNumericRequirement(
  label: string,
  competitorValue: number | undefined,
  wyrestormValue: number | undefined,
  maxPoints: number,
...
function _scoreRankedRequirement(
  label: string,
  competitorRank: number,
  wyrestormRank: number,
  competitorValue: string | undefined,
...
function _formatCapabilityFlags<T extends object>(
  profile: T | undefined,
  definitions: ReadonlyArray<{ key: keyof T; label: string }>,
): string {
...
function _statusForCapabilityFlags<T extends object>(
  competitorProfile: T | undefined,
  wyrestormProfile: T | undefined,
  definitions: ReadonlyArray<{ key: keyof T; label: string }>,
  missingLabel: string,
...
function _formatNumericCapability(value?: number, unit = ""): string {
  const numeric = Number(value) || 0;
  if (numeric <= 0) return "-";
  return unit ? `${numeric} ${unit}` : String(numeric);
}

function _statusForNumericCapability(
  competitorValue: number | undefined,
  wyrestormValue: number | undefined,
  label: string,
): { status: CompetitorCompareMatrixStatus; note?: string } {
...
function _formatTransportDetail(product: CatalogProduct): string {
  const parts = [
...
function _statusForTransportDetail(
...
function _formatPorts(ports?: CatalogPortCount[]): string {
  const values = (ports ?? [])
    .map((port) => {
...
function _formatComparablePorts(ports?: CatalogPortCount[]): string {
  const values = (ports ?? [])
    .filter((port) => !isWorkflowPortType(port.type))
    .map((port) => {
      const type = tidy(port.type);
...
function _formatVideo(product: CatalogProduct): string {
  const parts = [
...
function _formatControl(control?: string[]): string {
  const values = dedupeStrings(control ?? [], 6);
  return values.length > 0 ? values.join(", ") : "-";
}

function _formatDistance(product: CatalogProduct): string {
  return Number.isFinite(product.distance?.meters)
    ? `${product.distance?.meters}m`
    : "-";
}

function _statusForPortCoverage(
...
function _yesNoCellValue(value: boolean): string {
  return value ? "Yes" : "No";
}
...
function matchBasisSummary(
  method: CompetitorCompareMatchMethod,
  _ioBreakdown: CompetitorCompareIoBreakdown,
  _workflowCoverage: number,
  _specCoverage: number,
): string {
  switch (method) {
    case "direct-replacement":
