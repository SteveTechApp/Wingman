...
const FEATURE_NORMALIZATION: Record<string, ProductFeature> = {
  "usb-c": "USB-C",
  "usb c": "USB-C",
  hdmi: "HDMI",
...
function inferFeatures(text: string, existing: string[]): string[] {
  ...
  const add = (feature: ProductFeature, match: boolean) => {
...
export function enrichProductClassification(raw: RawProductRecord): ClassificationRecord {
  ...
  const commercialFamily =
    clean(raw.family ?? raw.series) || detectCommercialFamily(text);
...
export function isTraditionalMatrix(record: ClassificationRecord): boolean {
  return record.trueMatrix || record.routingClass === "Full Matrix";
}

export function isHybridRoomCore(record: ClassificationRecord): boolean {
  return (
...
export function shouldHideHybridInMatrixView(
  ...
  return !showHybridProducts && selectedFamily === "Matrix Switcher" && isHybridRoomCore(record);
}