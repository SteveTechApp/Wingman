...
export const PRODUCT_POSITIONING_CARDS: ProductPositioningCard[] = [...CORE_PRODUCT_POSITIONING_CARDS,...EXPANDED_PRODUCT_POSITIONING_CARDS,
];
...
export function searchProductPositioningCards(query: string): ProductPositioningCard[] {
 ...
 card.technologyType,
 card.salientPoint,
 card.oneLinePositioning,...card.bestFitApplications,...card.customerProblems,...card.listenForTriggers,...card.attachProducts.map((item) => `${item.sku?? ""} ${item.productFamily?? ""} ${item.reason}`),...card.competitorAngles.flatMap((item) => [
 item.competitorBrand?? "",
 item.competitorCategory?? "",
 item.positioningNote,...item.compareSearchTerms,
 ]),
 ].join(" ").toLowerCase();
