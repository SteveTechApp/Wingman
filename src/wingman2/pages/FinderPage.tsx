...
const FINDER_AUTO_LOAD_DISCOVERY_BRIEF = true;
...
function cleanDisplayText(value: unknown) {
 return String(value?? "").replace(/&amp;/gi, "&").replace(/&#x2122;|&#8482;|&trade;/gi, " TM ").replace(/&#x00ae;|&#174;|&reg;/gi, " R ").replace(/&#x2013;|&#8211;|&ndash;/gi, "-").replace(/&#x2014;|&#8212;|&mdash;/gi, "-").replace(/&#x2018;|&#8216;|&lsquo;/gi, "'").replace(/&#x2019;|&#8217;|&rsquo;/gi, "'").replace(/&#x201c;|&#8220;|&ldquo;/gi, '"').replace(/&#x201d;|&#8221;|&rdquo;/gi, '"').replace(/&#x2022;|&#8226;|&bull;/gi, "-").replace(/&nbsp;/gi, " ").replace(/&quot;/gi, '"').replace(/&#39;|&#x27;/gi, "'").replace(/[\u2013\u2014]/g, "-").replace(/[\u2018\u2019]/g, "'").replace(/[\u201c\u201d]/g, '"').replace(/\u2022/g, "-").replace(/[\u0080-\u024f]+/g, " ").replace(/[^\u0020-\u007e]/g, " ").replace(/\s+/g, " ").trim();
}
...
function deepText(value: unknown, depth = 0): string {
 ...
 return Object.values(value as UnknownRecord).map((item) => deepText(item, depth + 1)).join(" ");
 }
 return "";
}
...
function cleanFinderProduct(product: FinderProduct): FinderProduct {
 const cleaned: FinderProduct = {...product,
...
function normaliseIndexProduct(record: UnknownRecord): FinderProduct | null {
 ...
 (technicalProfile && typeof technicalProfile === "object"? asSalesLanguage((technicalProfile as UnknownRecord).salesLanguage): undefined);
 const tags = unique([
 family,
 getFirstString(record, ["category", "type", "technology"]),...deepText(record).split(/\s+/).filter((word) => /^(usb|hdbaset|hdmi|avoip|networkhd|ndi|4k|8k|matrix|video|wall|wireless|dante|audio|control|rs232)$/i.test(word)).slice(0, 10),
 ]);
 ...
 showWhenRequestedBy: Array.isArray(record.showWhenRequestedBy)? record.showWhenRequestedBy.map(String).filter(Boolean): [],
 };
...
function normaliseProductIndex(data: unknown) {
 ...
 bySku.set(key, {...existing,...product,
 tags: unique([...existing.tags,...product.tags]),
 searchText: cleanDisplayText(`${existing.searchText} ${product.searchText}`),
 });
 });
...
function getActiveFeatureFilters(need: FinderNeed) {
 return (Object.keys(need) as (keyof FinderNeed)[]).map((key) => getFeatureFilterForNeedField(key, need[key])).filter((filter): filter is FinderFeatureFilter => Boolean(filter));
}
...
function finderFeatureMatches(
 ...
 return products.map(cleanFinderProduct).filter((product) => isAllowedFeatureSearchProduct(product, need)).map((product) => applyFinderRequirementGate(toFeatureSearchMatch(product, need, filters, strictMatch, eligibilityNeed), need)).filter((match) => match.score >= 0).sort((a, b) => b.score - a.score || a.sku.localeCompare(b.sku));
}
...
function buildFinderMatchPlan(products: FinderProduct[], need: FinderNeed, hasIntent: boolean): FinderMatchPlan {
 ...
 const matches = products.map((product) => applyFinderRequirementGate(applyFinderVisibilityToMatch(scoreProduct(product, need), need), need)).filter((match) => shouldShowMatch(match, need)).sort(compareProductMatches).slice(0, hasPointToPointOneInOneOutNeed(need)? 4: 8);
 ...
 const architectureProducts = coreFilters.length? cleanProducts.filter((product) => coreFilters.every((filter) => filter.matches(product))): [];
 ...
 coreFilters[0];
 const closestProducts = primaryFilter? cleanProducts.filter((product) => primaryFilter.matches(product)): [];
...
function suppressedFinderClassMatch(product: FinderProduct): ProductMatch {
 return {...cleanFinderProduct(product),
 score: -999,
 status: "caution",
 };
}
...
function isSimpleHdmiSwitcherOnly(product: FinderProduct) {
 ...
 text.includes("hdmi switcher") &&!text.includes("mst") &&!text.includes("dual display") &&!text.includes("multi output") &&!text.includes("usb c")
 );
}
...
function getReasonLines(match: ProductMatch, need: FinderNeed) {
 const lines: string[] = [];
 const path = inferPathFromNeed(need);
 const featureLabels = getActiveFeatureFilters(need).filter((filter) => filter.matches(match)).map((filter) => filter.label);
...
function addProductToProject(projectId: string, product: ProductMatch | FinderProduct, need?: FinderNeed) {
 ...
 writeProductSelections({...selections,
...
function createProjectFromProduct(projectName: string, owner: string, product: ProductMatch | FinderProduct, need?: FinderNeed) {
 ...
 writeProductSelections({...readProductSelections(),
 [id]: [selection],
 });
 return project;
}
...
function ChipButton({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
 ...
 active? "border-slate-950 bg-slate-950 text-white": "border-[#29465e] bg-[#0d2133] text-white/70 hover:border-cyan-300 hover:bg-cyan-50 hover:text-cyan-800"
 }`}
...
function StatusPill({ status }: { status: MatchStatus }) {
 ...
 const classes =
 status === "recommended"? "bg-emerald-100 text-emerald-700": status === "alternative"? "bg-cyan-100 text-cyan-700": "bg-rose-100 text-rose-700";
...
useEffect(() => {
 ...
 loadProductIntelligenceIndex().then((data) => {
 if (!active) return;
 ...
 setNeed((current) => ({...current,...draftNeed,
 }));
...
if (!leadingMatch) {
 ...
 const finderResultsSummary =!hasIntent? "Add a starting requirement to generate recommendations.": finderMatchPlan.mode === "exact"? `${rankedMatches.length} exact product match${rankedMatches.length === 1? "": "es"} from the current Finder need.`: finderMatchPlan.mode === "architecture"? `${rankedMatches.length} core product path${rankedMatches.length === 1? "": "s"} match the discovered architecture.`: finderMatchPlan.mode === "closest"? `${rankedMatches.length} closest product path${rankedMatches.length === 1? "": "s"}; validate the remaining technical detail before quoting.`: `${rankedMatches.length} matching product${rankedMatches.length === 1? "": "s"} from the current Finder need.`;
 ...
 isActive? "border-slate-950 bg-slate-950 text-white shadow-lg": isComplete? "border-emerald-200 bg-emerald-50 text-[#edf6ff] hover:border-emerald-300": "border-[#29465e] bg-[#0d2133] text-white/70 hover:border-cyan-300 hover:bg-cyan-50"
 }`}
 >
 <span
