...
export function WingmanGuruFab({ open, onClick }: WingmanGuruFabProps) {
  ...
  function handlePointerMove(event: PointerEvent<HTMLButtonElement>) {
    ...
    setPosition(buttonPosition(event.currentTarget, drag.startLeft + dx, drag.startTop + dy));
  }
  ...
  function handlePointerUp(event: PointerEvent<HTMLButtonElement>) {
    ...
    if (drag.moved) {
      const rect = event.currentTarget.getBoundingClientRect();
      const nextPosition = buttonPosition(event.currentTarget, rect.left, rect.top);
      setPosition(nextPosition);
      saveStoredPosition(nextPosition);
