...
const NetworkInfrastructurePanel: React.FC = () => {
    ...
    const handleChange = (name: keyof ProjectInfrastructure, checked: boolean) => {
        const newInfra: ProjectInfrastructure = {
  ...infrastructure,
  hasLocalRacks: (infrastructure as any)?.hasLocalRacks ?? false,
  hasCentralizedInfrastructure: (infrastructure as any)?.hasCentralizedInfrastructure ?? false,
  [name]: checked,
};
