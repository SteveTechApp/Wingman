...
export function getGuruContext(pathname: string): GuruContext {
  ...
  if (path.includes("/discovery")) {
    return {
      ...
      quickPrompts: [
        "What AV Guide inputs are still missing?",
        "Which WyreStorm family is most likely from the captured inputs?",
  ...
  if (path.includes("/dashboard")) {
    return {
      ...
      suggestions: [
        ...
        "Complete AV Guide before selecting products or generating proposals.",
