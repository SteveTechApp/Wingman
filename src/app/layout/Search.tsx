    ...
    return (
        <>
            ...
            >
                ...
                <span>Search...</span>
                <kbd className="ml-auto\ text-xs\ font-mono\ bg-background\ border\ border-border-color\ rounded\ px-1\.5\ py-0\.5">?f��??s��?,??"?f??s�?f��??s��?,??"?f��?,?s�?f��?,?z��?,???o�??s���?,???o�??s�?.??oK</kbd>
            </button>
            ...
            <InfoModal isOpen={isOpen} onClose={handleCloseModal} className="max-w-2xl" title={null}>
                ...
                <div className="mt-4\ max-h-\[60vh]\ overflow-visible">
                    ...
                    {results.training.length > 0 && (
                        <div className="mb-4">
                            ...
                            {results.training.map(t => <SearchResultItem key={t.id} title={t.title} category="Training" onClick={() => handlePageClick('/app/tools/training')} />)}
                        </div>
                    )}
