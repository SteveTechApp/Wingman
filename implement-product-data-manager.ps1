...
#
# This script:
#   1. Confirms the Wingman repo
#   2. Checks Git state
#   3. Creates a dedicated implementation branch
#   4. Creates a local backup snapshot
#   5. Writes a detailed Codex implementation brief
#   6. Runs Codex against the existing repository
#   7. Runs typecheck / tests / build
#   8. Shows the resulting Git diff/status
#
# It does NOT automatically commit or push.
...
$Branch = "feature/product-data-governance"
$Timestamp = Get-Date -Format "yyyyMMdd-HHmmss"

$WorkDir = Join-Path $Repo ".wingman-work"
$BackupDir = Join-Path $WorkDir "backups\$Timestamp"
$BriefFile = Join-Path $WorkDir "product-data-manager-pass.md"
...
if (-not (Test-Path $Repo)) {
    throw "Wingman repository not found at $Repo"
}
...
if ($dirty) {
    Write-Host ""
    Write-Host "WARNING: Existing uncommitted changes detected." -ForegroundColor Yellow
    Write-Host "They will NOT be deleted or reset." -ForegroundColor Yellow
    Write-Host ""
    git status --short
    Write-Host ""
    Write-Host "The script will continue, but the final diff may include earlier work." -ForegroundColor Yellow
}
...
Step "3. Creating / selecting implementation branch"
...
Step "4. Creating local backup snapshot"
...
# ---------------------------------------------------------------------
# 5. WRITE CODEX IMPLEMENTATION BRIEF
# ---------------------------------------------------------------------

Step "5. Creating implementation brief"
...
Write-Host "Implementation brief created:" -ForegroundColor Green
...
if (-not $codex) {
    ...
    Write-Host ""
    Write-Host "The implementation brief has still been created successfully:"
    Write-Host $BriefFile -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Open your coding agent in this repository and give it that file."
    Write-Host "No repository files have been modified by Codex."
    exit 0
}

Write-Host "Codex found: $($codex.Source)" -ForegroundColor Green

# ---------------------------------------------------------------------
# 7. RUN CODEX
# ---------------------------------------------------------------------

Step "7. Running Wingman implementation pass"

$prompt = @"
Read the implementation specification at:

$BriefFile

Work directly in the current Wingman repository:

$Repo

Inspect the existing architecture before editing.

Implement the specification as one controlled development pass.

Do not commit.
Do not push.
Do not delete unrelated functionality.
Do not reset existing user changes.

Run the required validation commands before finishing.
"@

# codex exec receives the complete development instruction.
codex exec $prompt

if ($LASTEXITCODE -ne 0) {
    Write-Host ""
    Write-Host "Codex exited with an error." -ForegroundColor Red
    Write-Host "No automatic reset will be performed." -ForegroundColor Yellow
    Write-Host "Backup is available at:" -ForegroundColor Yellow
    Write-Host $BackupDir
    exit $LASTEXITCODE
}
...
Step "8. Running TypeScript validation"
...
if ($package.scripts.typecheck) {
    ...
    Write-Host "No npm run typecheck script found." -ForegroundColor Yellow
}
...
if ($package.scripts.test) {
    ...
    Write-Host "No npm test script found." -ForegroundColor Yellow
}
...
Step "10. Running production build"
...
if ($package.scripts.build) {
    ...
    Write-Host "No npm run build script found." -ForegroundColor Yellow
}

# ---------------------------------------------------------------------
# 11. FULL VERIFY IF AVAILABLE
# ---------------------------------------------------------------------

if ($package.scripts.verify) {

    Step "11. Running Wingman full verification"
...
Step "12. Implementation complete - review changes"
...
Write-Host ""
Write-Host "Implementation brief:" -ForegroundColor Cyan
Write-Host $BriefFile
...
Write-Host ""
Write-Host "DO NOT COMMIT YET." -ForegroundColor Yellow
Write-Host ""
Write-Host "Next:"
Write-Host "  1. npm run dev"
Write-Host "  2. Open Wingman in the browser"
Write-Host "  3. Test Product Data Manager"
Write-Host "  4. Test Finder"
Write-Host "  5. Test Compare"
Write-Host "  6. Test Discovery"
Write-Host "  7. Check git diff"
Write-Host ""
