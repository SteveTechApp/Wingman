...
function Save-Utf8NoBom { param([string]$Path,[string]$Content) $enc = New-Object System.Text.UTF8Encoding($false); [System.IO.File]::WriteAllText($Path,$Content,$enc) }
function Backup-File { param([string]$Path) if (Test-Path $Path) { $stamp=Get-Date -Format "yyyyMMdd-HHmmss"; Copy-Item -LiteralPath $Path -Destination "$Path.bak-$stamp" -Force; Write-Host "Backup created: $Path.bak-$stamp" -ForegroundColor DarkGray } }

$root = (Get-Location).Path
$serverPath = Join-Path $root "server\competitor-lookup-server.mjs"
$builderDir = Join-Path $root "server\intelligence"
$builderPath = Join-Path $builderDir "auto-draft-builder.mjs"
$clientApiPath = Join-Path $root "src\wingman2\data\intelligenceDraftApi.ts"

Write-Step "Checking files"
if (-not (Test-Path $serverPath)) { throw "Missing server file: $serverPath" }
if (-not (Test-Path (Join-Path $root "server\competitor\compare-intelligence.mjs"))) { throw "Missing compare intelligence engine." }
if (-not (Test-Path $builderDir)) { New-Item -ItemType Directory -Path $builderDir | Out-Null }
Backup-File $serverPath
Backup-File $builderPath
Backup-File $clientApiPath

Write-Step "Writing backend automatic draft builder"
$builder = @'
...
Save-Utf8NoBom -Path $builderPath -Content $builder

Write-Step "Writing frontend API helper"
$client = @'
...
'@
Save-Utf8NoBom -Path $clientApiPath -Content $client

Write-Step "Patching server route registration"
$server = Get-Content -LiteralPath $serverPath -Raw
$server = $server -replace "`r`n", "`n"

if ($server -notmatch "auto-draft-builder") {
  $server = $server.Replace(
    'import { resolveCompetitorLiveLookup } from "./competitor/live-lookup.mjs";',
    'import { resolveCompetitorLiveLookup } from "./competitor/live-lookup.mjs";' + "`n" +
    'import {' + "`n" +
    '  handleIntelligenceDraftBuildCompetitorPost,' + "`n" +
    '  handleIntelligenceDraftBuildWyrestormPost,' + "`n" +
    '  handleIntelligenceDraftsGet,' + "`n" +
    '  handleIntelligenceDraftStatusPost,' + "`n" +
    '} from "./intelligence/auto-draft-builder.mjs";'
  )
}

if ($server -notmatch "intelligenceDraftsEndpoint") {
  $server = $server.Replace(
    'productIntelligenceHealthEndpoint: `http://${HOST}:${PORT}/api/product-intelligence/health`,',
    'productIntelligenceHealthEndpoint: `http://${HOST}:${PORT}/api/product-intelligence/health`,' + "`n" +
    '    intelligenceDraftsEndpoint: `http://${HOST}:${PORT}/api/intelligence/drafts`,' + "`n" +
    '    competitorDraftBuildEndpoint: `http://${HOST}:${PORT}/api/intelligence/build-competitor`,' + "`n" +
...
if ($server -notmatch '"/api/intelligence/build-competitor"') {
  $server = $server.Replace('  if (method === "GET" && url.pathname === "/api/product-intelligence/health") {', $routeBlock + '  if (method === "GET" && url.pathname === "/api/product-intelligence/health") {')
}

if ($server -notmatch "handleIntelligenceDraftBuildCompetitorPost") { throw "Failed to patch server import." }
if ($server -notmatch '"/api/intelligence/build-competitor"') { throw "Failed to patch server route." }
...
Write-Host ""
Write-Host "Automatic Product Intelligence Draft Builder installed." -ForegroundColor Green
Write-Host ""
Write-Host "Run server:"
Write-Host "npm run server:dev"
Write-Host ""
Write-Host "Test competitor build in another PowerShell window:"
Write-Host 'Invoke-RestMethod -Method Post -Uri "http://127.0.0.1:8787/api/intelligence/build-competitor" -ContentType "application/json" -Body (@{ brand="Kramer"; sku="VP-440X"; allowWeb=$true } | ConvertTo-Json)'
Write-Host ""
Write-Host "Test WyreStorm build:"
Write-Host 'Invoke-RestMethod -Method Post -Uri "http://127.0.0.1:8787/api/intelligence/build-wyrestorm" -ContentType "application/json" -Body (@{ sku="MX-0403-H3-MST" } | ConvertTo-Json)'
