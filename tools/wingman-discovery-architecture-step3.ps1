...
$rescue = Join-Path $RepoRoot "_RESCUE_DiscoveryArchitectureStep3_$stamp"
...
/* ===== Wingman discovery architecture step 3 ===== */
...
.wm-dw5 .wm-ui__subtitle{
  margin:10px 0 0;
  max-width:860px;
  color:rgba(236,244,255,0.76);
}

.wm-dw5 .wm-ui__sectionTitle{
  margin:0;
}

.wm-dw5 .wm-ui__sectionText{
  margin:6px 0 0;
  color:rgba(236,244,255,0.72);
}

.wm-dw5 .wm-ui__miniGrid{
  display:grid;
  grid-template-columns:repeat(2,minmax(0,1fr));
  gap:12px;
}

.wm-dw5 .wm-ui__miniItem{
  display:flex;
  flex-direction:column;
  gap:6px;
  padding:12px;
...
if ($layoutRaw -notmatch 'Wingman discovery architecture step 3') {
  $layoutRaw = $layoutRaw.TrimEnd() + "`r`n`r`n" + $layoutBlock.Trim() + "`r`n"
  Save-Utf8NoBom -Path $layoutPath -Content $layoutRaw
}

Write-Host ""
Write-Host "Wingman discovery architecture step 3 applied." -ForegroundColor Green
