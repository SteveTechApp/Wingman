...
const primaryButtonCount = (
  dashboard.match(
    /data-wingman-dashboard-primary-button="true"/g,
  ) ?? []
).length;

if (primaryButtonCount !== 2) {
  errors.push(
    `Expected exactly 2 visible Dashboard primary rail buttons; found ${primaryButtonCount}.`,
  );
}
...
[
  ...
  if (!dashboard.includes(marker)) {
    errors.push(
      `DashboardPage.tsx missing primary button marker: ${marker}`,
    );
  }
});
...
[
  ...
  if (!css.includes(marker)) {
    errors.push(
      `wingman-style-stack.css missing primary button marker: ${marker}`,
    );
  }
});
...
[
  ...
  if (dashboard.includes(marker) || css.includes(marker)) {
    errors.push(
      `Obsolete primary-button marker is still present: ${marker}`,
    );
  }
});
...
console.log(
  "[dashboard-primary-buttons] Verified current visible Dashboard primary actions.",
);