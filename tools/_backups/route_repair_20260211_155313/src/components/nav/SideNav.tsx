...
export default function SideNav() {
  ...
  const sections = useMemo(() => {
    const workspace: NavItem[] = [
      { to: "/dashboard", label: "Dashboard", desc: "Overview + quick access" },
      { to: "/projects", label: "Projects", desc: "Create, open, manage projects" },
    ];

    const project: NavItem[] = [
      { to: "/import", label: "Import Intake", desc: "Bring in requirements / files" },
    ];

    const tools: NavItem[] = [
      { to: "/toolhub", label: "Tool Hub", desc: "All tools in one place" },
      { to: "/tools/competitor-compare", label: "Competitor Compare", desc: "Match SKUs + alternatives" },
    ];
  ...
  const showProjectHint = useMemo(() => {
    ...
    return p.startsWith("/import") || p.startsWith("/toolhub") || p.startsWith("/tools/competitor-compare");
  }, [loc.pathname]);
