...
$backupDir = "C:\Users\steve\wingman\_RESCUE\fix-video-wall-safe-fallbacks-$stamp"
...
'  const useCaseLabel = wallType === "led" ? "LED deployment" : "LCD deployment";',
'',
...
for ($j = 0; $j -lt $fallbackBlock.Count; $j++) {
  $lines.Insert($start + $j, $fallbackBlock[$j])
}
...
Write-Host ""
Write-Host "Replaced recommendation-dependent block with safe fallbacks."
Write-Host "Backup saved to: $backupDir"
Write-Host ""
Write-Host "Now run:"
Write-Host "  npm run typecheck"
Write-Host ""
Write-Host "If the page still errors, run:"
Write-Host '  Get-Content C:\Users\steve\wingman\src\features\misc\VideoWallPlannerPage.tsx | Select-Object -Skip 660 -First 140'