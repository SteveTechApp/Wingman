...
$backupRoot = Join-Path $repoRoot "_RESCUE\compare-manual-panel-collapsible-fix-$stamp"
New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null
Backup-File -Path $path -RepoRoot $repoRoot -BackupRoot $backupRoot

$t = Read-Text $path

if ($t -notmatch '\bmanualPanelOpen\b') {
  $insertState = @'
...
if ($t -notmatch '\bmanualPanelOpen\b') {
  throw "Could not insert manualPanelOpen state."
}

$pattern = '(?s)<div className="wm-card"(?:\s+style=\{\{[^}]*\}\})?>\s*<CompetitorManualComparisonPanel\s+brand=\{brand\}\s+query=\{sku\}\s+selectedCandidate=\{selectedCandidate\}\s+selectedOption=\{selectedOption\}\s+saveMessage=\{saveMessage\}\s+feedbackSummary=\{feedbackSummary\}\s+onSave=\{handleSaveManual\}\s+onDelete=\{handleDeleteManual\}\s*/>\s*</div>'
...
if (-not $rx.IsMatch($t)) {
  throw "Could not locate CompetitorManualComparisonPanel card. Run: Get-Content .\src\features\compare\CompetitorComparePage.tsx | Select-Object -Skip 440 -First 80"
}
