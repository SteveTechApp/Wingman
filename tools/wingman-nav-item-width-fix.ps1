# tools\wingman-nav-item-width-fix.ps1
...
$backupRoot = Join-Path $Root "_RESCUE\nav-item-width-fix-$stamp"
New-Item -ItemType Directory -Force -Path $backupRoot | Out-Null
Backup-File -Path $cssPath -BackupRoot $backupRoot -RootPath $Root

$css = Get-Content $cssPath -Raw

$startMarker = "/* WINGMAN_NAV_ITEM_WIDTH_FIX_START */"
$endMarker   = "/* WINGMAN_NAV_ITEM_WIDTH_FIX_END */"
...
$updated = Replace-OrAppendBlock -Content $css -StartMarker $startMarker -EndMarker $endMarker -NewBlock $block
Save-Utf8NoBom -Path $cssPath -Content $updated

Write-Host ""
Write-Host "Applied width-only nav item fix." -ForegroundColor Green
Write-Host "Updated:"
Write-Host " - $cssPath"
Write-Host "Backup:"
