...
$decisionBoundaryPath = Join-Path $root "src\wingman2\lib\recommendationsDecisionBoundary.ts"
...
$decisionBoundaryContent = Read-Utf8NoBom $decisionBoundaryPath
...
if (-not $decisionBoundaryContent.Contains('from "./recommendationSafety"')) {
    $decisionBoundaryContent =
        'import { recommendationCandidateAllowed } from "./recommendationSafety";' +
        "`n" +
        $decisionBoundaryContent
}

$oldBoundaryCandidateBlock =
    "    candidates: slot.supply === `"external`" ? [] : slotPool`n" +
    "      .filter((decision) => decision.eligible)`n" +
    "      .filter((decision) => productMatchesSlot(classification(decision), slot))`n" +
    "      .slice(0, 4),`n"

$newBoundaryCandidateBlock =
    "    candidates: slot.supply === `"external`" ? [] : slotPool`n" +
    "      .filter((decision) => decision.eligible)`n" +
    "      .filter((decision) => productMatchesSlot(classification(decision), slot))`n" +
    "      .filter((decision) => recommendationCandidateAllowed(`n" +
    "        decision,`n" +
    "        slot.kind,`n" +
    "        [brief?.roomModel, brief],`n" +
    "      ))`n" +
    "      .slice(0, 4),`n"

$decisionBoundaryContent = Replace-Once `
    $decisionBoundaryPath `
    $decisionBoundaryContent `
    $oldBoundaryCandidateBlock `
    $newBoundaryCandidateBlock `
    "Gate resolver slot candidates by pairing and dependency safety"
...
Add-FilePatch -Changes $changes -Path $decisionBoundaryPath -Content $decisionBoundaryContent -Description "Patch recommendation decision boundary"
