...
const shortLabels = [
  ...dashboard.matchAll(
    /data-wingman-dashboard-short-label="([^"]+)"/g,
  ),
].map((match) => match[1]);

if (shortLabels.join(",") !== "Discover,Compare") {
  errors.push(
    `Expected short labels Discover,Compare on visible rail actions; found ${
      shortLabels.join(",") || "none"
    }.`,
  );
}
...
[
  ...
  if (!dashboard.includes(marker)) {
    errors.push(
      `DashboardPage.tsx missing short-button marker: ${marker}`,
    );
  }
});
...
[
  ...
  if (!css.includes(marker)) {
    errors.push(
      `wingman-style-stack.css missing short-button marker: ${marker}`,
    );
  }
});
...
[
  ...
  if (dashboard.includes(marker) || css.includes(marker)) {
    errors.push(
      `Obsolete short-button marker is still present: ${marker}`,
    );
  }
});
...
console.log(
  "[dashboard-short-buttons] Verified visible compact rail button labels.",
);