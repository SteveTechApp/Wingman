...
# ---------------------------
# 1) Add exports expected by other code: getProductConsolidationLogic
# ---------------------------
$pcPath = Join-Path $RepoRoot "src\services\prompts\designRoom\productConsolidation.ts"
if (Test-Path -LiteralPath $pcPath) {
  $txt = Get-Content -LiteralPath $pcPath -Raw
  if ($txt -notmatch "getProductConsolidationLogic") {
    BackupFile $pcPath
    $txt2 = $txt.TrimEnd() + "`r`n`r`n" + @'
export function getProductConsolidationLogic(): string {
  return productConsolidationPrompt;
}
'@ + "`r`n"
    WriteUtf8NoBom $pcPath $txt2
  } else {
    Write-Host "OK: getProductConsolidationLogic already present." -ForegroundColor Green
  }
} else {
  Write-Host "WARN: productConsolidation.ts not found; skipped export shim." -ForegroundColor Yellow
}

# ---------------------------
# 2) Expand PasswordManager to match AuthService expectations (thin wrappers)
# ---------------------------
...
'@
  ...
  WriteUtf8NoBom $pmPath $pm
} else {
  Write-Host "WARN: PasswordManager.ts not found; skipped." -ForegroundColor Yellow
}

# ---------------------------
# 3) Create a scoped tsconfig for typecheck (exclude legacy/experimental drift)
# -----------
