...
export async function refreshLiveProductData(options?: {
  ...
  refreshPromise = Promise.race<LiveProductDataSnapshot | null>([
    fetchProductIntelligenceRecords({ limit: 1000 })
      .then((result) => setSnapshot(toSnapshot(result)))
      .catch((error) => {
        console.warn("[Wingman] refreshLiveProductData failed; using cached snapshot.", error);
        return current ?? null;
      }),
    new Promise<LiveProductDataSnapshot | null>((resolve) => {
      window.setTimeout(() => {
        console.warn("[Wingman] refreshLiveProductData timed out; using cached snapshot.");
