...
const outputTargets = [
  {
    type: "json",
    path: "src/generated/product-intelligence-index.json",
  },
  {
    type: "ts",
    path: "src/generated/product-intelligence-index.ts",
  },
  {
    type: "json",
    path: "src/data/product-intelligence-index.json",
  },
  {
    type: "ts",
    path: "src/data/product-intelligence-index.ts",
  },
  {
    type: "json",
    path: "public/product-intelligence-index.json",
  },
];
...
function normalizeProduct(item, index, sourceFile) {
  ...
  const features = Array.isArray(item?.features)
    ? item.features.map((feature) =>
        typeof feature === "string" ? feature.trim() : asString(feature?.name || feature?.label || feature)
      ).filter(Boolean)
    : [];
...
function buildIndex(products, discoveredSources) {
  ...
  for (const product of products) {
    if (product.sku) {
      bySku[product.sku] = product.id;
    }

    if (product.name) {
      byName[product.name] = product.id;
    }
  }
