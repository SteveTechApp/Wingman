# Product story coverage backlog

> Generated by `node tools/audit-product-story-coverage.mjs`. Do not edit by hand.
> Coverage is measured against ACTIVE 2026 SKUs only; SKUs are alias-resolved.

- Governed stories: **111** (reviewed **23**, catalogue-grounded **88**)
- Active catalogue SKUs (alias-deduped): **99**
- Active covered: **99 (100%)** · Active uncovered: **0**
- Excluded (deliberately not storied): cable **18**, discontinued **12**, do-not-spec **16**, unlisted **5**

## Active SKUs still needing a governed story

_None — every active SKU has a governed story._

## Unlisted SKUs (in the catalogue but on no 2026 business list)

Confirm whether these are active before authoring a story; the compare engine treats them as unresolved risk.

- [ ] APO-210-UC
- [ ] EX-100-G2
- [ ] EX-100-IW-USBC
- [ ] EX-80-KVM
- [ ] NETWORKHD TOUCH TM

## Human-review backlog (catalogue-grounded stories)

88 stories were generated from the official catalogue spec and adversarially verified, but still want a human spot-check against the current datasheet. Find them by searching `provenance: "catalogue-grounded"` in `src/wingman2/data/productStories.ts`.
