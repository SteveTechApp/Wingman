...
- [ ] Write failing tests proving requirement interpretation, role coverage, contextual product explanations, blockers and deterministic hash.
- [ ] Run `npx vitest run src/wingman2/lib/designProposal.test.ts` and confirm failure.
- [ ] Implement normalized types, role inference, requirement trace, completeness evaluation and hash.
- [ ] Add the optional revision to the persisted proposal shape without breaking older records.
...
- [ ] Write failing tests for revision persistence and approval invalidation after requirement/product changes.
- [ ] Implement commands through `updateStoredProject`; do not add a second storage authority.
- [ ] Make proposal save refresh the canonical revision.
...
- [ ] Write a rendered test for the full requirement-to-product trace and missing-role warning.
- [ ] Implement the review UI and place it in the wizard’s review/export step.
- [ ] Ensure responsive layout and visible keyboard focus.
- [ ] Run rendered test and typecheck.
...
- [ ] Add failing output tests for canonical sections and design revision identifier.
- [ ] Render canonical sections in DOCX and shared HTML/PDF markup.
- [ ] Block customer output when the revision has blocking gaps or an approval hash is stale.
- [ ] Update the outdated proposal contract markers to assert current behaviour.
- [ ] Run proposal tests and `npm run check:proposal-wizard`.
...
- [ ] Define six complete scenario fixtures with explicit expected requirements, roles, products and blockers.
- [ ] Assert complete scenarios can issue and incomplete scenarios identify the exact missing roles.
- [ ] Assert product explanations name requirement and design role.
...
- [ ] Run focused design/proposal/workflow tests.
- [ ] Run `npm run typecheck`, `npm run check:workflow`, `npm run check:proposal-wizard`, and `npm run build`.
- [ ] Mark completed checklist items and record any remaining warnings.
