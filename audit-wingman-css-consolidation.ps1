...
for ($i = 0; $i -lt $mainLines.Count; $i++) {
    if ($mainLines[$i] -match '^\s*import\s+["''](.+?\.css)["''];?\s*$') {
...
# Memory is safe here because active CSS is ~1 MB total, not the multi-GB archive.
$selectorRows = @()
...
foreach ($f in $cssFiles) {
    ...
    $textNoComments = [regex]::Replace($text, '(?s)/\*.*?\*/', '')

    # Identify selector headers immediately preceding an opening brace.
    # Exclude at-rules such as @media/@supports/@keyframes.
    $matches = [regex]::Matches($textNoComments, '(?ms)(^|[}])\s*([^@{}][^{}]*?)\s*\{')
    ...
    foreach ($m in $matches) {
        ...
        foreach ($part in ($selector -split ',')) {
            $normalized = ([regex]::Replace($part.Trim(), '\s+', ' ')).Trim()
...
Step "4. Finding historical patch / repair blocks"
...
Step "5. Finding repeated declarations of core UI selectors"
...
$topFiles = ($summary | Sort-Object KB -Descending | Select-Object -First 10 | ForEach-Object {
    "- $($_.File): $($_.Lines) lines / $($_.KB) KB / !important=$($_.Important)"
}) -join "`r`n"

$topDupes = ($selectorGroups | Select-Object -First 25 | ForEach-Object {
    "- ``$($_.Selector)`` — $($_.Occurrences) occurrences — $($_.Files)"
}) -join "`r`n"

$coreConflicts = if ($core.Count -gt 0) {
    ($core | ForEach-Object {
        "- ``$($_.Selector)`` — $($_.Occurrences) occurrences — $($_.Files)"
    }) -join "`r`n"
}
else {
    "- No core selector conflicts detected by the simple scanner."
}
...
$(
    if ($imports.Count -gt 0) {
        ($imports | ForEach-Object { "- line $($_.Line): ``$($_.Import)``" }) -join "`r`n"
    } else {
        "- No CSS imports detected."
    }
)
...
Write-Host ""
Write-Host "Paste the report here and the next pass can safely remove duplicated/legacy CSS."
