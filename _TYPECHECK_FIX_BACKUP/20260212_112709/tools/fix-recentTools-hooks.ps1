...
$path = "$Root\src\components\app\tools\recentTools.ts"
if (!(Test-Path $path)) { throw "Not found: $path" }
...
export function subscribeRecentTools(fn: Listener) { listeners.Add ? listeners.Add(fn) : listeners.add(fn); return () => listeners.delete(fn); }
...
# NOTE: PowerShell Set methods differ; keep it simple
# Replace the bad listeners.Add usage safely:
$code = $code -replace "listeners.Add \? listeners.Add\(fn\) : listeners.add\(fn\);", "listeners.add(fn);"

Write-Utf8NoBom $path $code
Write-Host "✔ Patched recentTools.ts to export useRecentTools + useUsageCounts"
