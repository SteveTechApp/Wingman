...
function RelPath([string]$Root, [string]$FullName) {
    # Compatible with both Windows PowerShell 5.1/.NET Framework and PowerShell 7+.
    ...
    # Fallback for paths outside the repo root.
    return $fullPath
}
...
Write-Host "Memory-safe mode enabled: large text files are scanned line-by-line." -ForegroundColor DarkGray
...
$loc = foreach ($f in $sourceFiles) {
    try {
        ...
        Write-Warning "Skipped source LOC scan for $($f.FullName): $($_.Exception.Message)"
    }
}
    ...
    try {
        ...
        try {
            while ($null -ne ($line = $reader.ReadLine())) {
                ...
                $mediaBlocks += ([regex]::Matches($line, '@media')).Count
            }
        }
        finally {
            $reader.Dispose()
        ...
        Write-Warning "Skipped CSS audit for $($f.FullName): $($_.Exception.Message)"
    }
}
...
$cssImports = foreach ($f in $codeFiles) {
    try {
        ...
        Write-Warning "Skipped CSS import scan for $($f.FullName): $($_.Exception.Message)"
    }
}
